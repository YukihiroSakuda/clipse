...
unsafe fn do_capture(cap_x: i32, cap_y: i32, cap_w: u32, cap_h: u32) -> Result<RgbaImage, String> {
    ...
    // A brand-new duplication's very first acquired frame can come back
    // completely black on some drivers/GPUs — a documented DXGI quirk. Since a
    // fresh `IDXGIOutputDuplication` is created on every call (no caching
    // across captures), this can hit *every single time* on affected hardware,
    // not just occasionally. Retry a few times against the *same* duplication
    // (which is primed after the first acquire) before giving up — this is far
    // less disruptive than silently dropping to the xcap/GDI fallback, which
    // loses physical-pixel resolution and (at the time of writing) any cursor
    // overlay support.
    ...
    for _attempt in 0..MAX_BLACK_RETRIES {
        ...
        eprintln!(
            "[dxgi] frame {}×{} fmt={} mon=({},{}) cap=({},{},{},{}) attempt={_attempt}",
        ...
        // Use the frame's actual format — CopySubresourceRegion silently
        // no-ops when source and destination formats differ.
        ...
        if let Err(e) = device.CreateTexture2D(&staging_desc, None, Some(&mut staging)) {
            dupl.ReleaseFrame().ok();
            return Err(format!("CreateTexture2D: {e}"));
        }
        ...
        return RgbaImage::from_raw(cw, ch, pixels)
