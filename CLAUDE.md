...
**Overlay windows are pooled, not per-capture.** They are prewarmed hidden at startup (`window::prewarm_overlays`) and kept alive (hidden) between captures, so PrintScreen only has to *show* them — webview creation (hundreds of ms) is off the hot path. `open_overlay` shows the pool when the monitor-layout signature (`AppState.overlay_signature`) still matches, emitting `overlay-show`; the overlay frontend re-fetches window lists/scroll mode/origin and resets interaction state on that event. A signature mismatch rebuilds the pool under a bumped generation label (avoids label collision with the still-closing old windows). Esc calls the `cancel_overlay` IPC which **hides** (not closes) them all; `window::{hide_all_overlays, close_all_overlays}` operate on every `overlay`-prefixed window. The capability `windows` list uses the `overlay-*` glob.

**A pool that doesn't cover every monitor is not a pool.** The failure this guards against is "the sub-monitor sometimes gets no overlay", and it had three ways in, all of them now closed in `window.rs`:

- **Two threads mutating the pool at once.** The capture path (`open_overlay_inner`, always on an async-runtime worker) and the display-change rebuild (`prewarm_overlays`, dispatched to the *main* thread by `hook_win`) both build/close/re-place overlays, so a `WM_DISPLAYCHANGE` landing next to a PrintScreen could close the very windows the capture had just decided to show. Both now run under `POOL_LOCK`, which covers the monitor enumeration too, since the pool is keyed on its result. **Nothing may block on that lock from the main thread** — creating a window from a worker waits *on* the main thread, so a main-thread waiter deadlocks the app; `prewarm_overlays` therefore takes it with `try_lock` and skips (harmlessly — the signature is cleared, so the next capture rebuilds).
- **A partial build kept as if it were whole.** A `WebviewWindowBuilder::build()` failure used to abort the loop, leaving every monitor after it bare (and, on the capture path, the earlier ones orphaned on screen). `build_pool` now keeps going, returns how many windows it actually made, and the callers **clear the signature** unless the count is complete — a one-off creation failure can no longer become a bare monitor for the rest of the session.
- **A pool accepted on count alone.** `close()` only *requests* teardown, so an older generation can still be listed for a frame or two; counting `overlay`-prefixed windows could accept a set that covers one monitor twice and another not at all. `usable_pool` takes only the newest generation and requires its indices to be exactly `0..monitor count`.

**The pool is enumerated from Win32, not from xcap** (`overlay_monitors`). `xcap::Monitor::all` builds each entry from `EnumDisplaySettingsW` *and* a `CreateDC` on the device and **silently skips any monitor either call fails on** — a monitor missing from that list gets no overlay window at all, with nothing downstream able to tell it was ever there. `EnumDisplayMonitors` + `GetMonitorInfoW` also puts the pool in `rcMonitor` space, which is what the rest of the app already works in (DXGI finds its output by `rcMonitor`, the recorder takes its origin from it, Tauri's `PhysicalPosition`/`PhysicalSize` are Win32 virtual-screen coordinates, and the overlay frontend derives every physical coordinate from its own window's `outerPosition()`), rather than in the display *mode's* geometry xcap reports — which for a rotated display is the unrotated one, i.e. a landscape-sized overlay on a portrait sub-monitor. Everything *downstream* of the overlay (`freeze_desktop`'s composite, `get_monitors`' hover targets) is still keyed on xcap, so `prewarm_overlays` compares the two lists off the hot path and logs any disagreement — that line is the only thing that would report the two spaces having drifted apart.

`place_overlay` sets the position, then the size, then **the position again**. Moving a window onto a monitor with a different scale factor makes Windows send `WM_DPICHANGED`, whose suggested rectangle tao applies for us — near where we asked, not where we asked — which on a mixed-DPI desktop leaves the overlay straddling the boundary right after the move meant to place it. Setters are queued to the main thread and don't block on it (unlike the getters this path must avoid), so the extra call is free on the hot path.
