...
- **Ctrl+V has two possible sources, and recency picks between them.** A paste can mean "the elements I copied in an editor" (the internal clipboard above) or "the picture on the system clipboard" (which becomes an `image` annotation — see the annotation system below). Neither is a safe fixed preference: every capture auto-copies its own image, so preferring the picture would make a screenshot outrank elements copied minutes later, while preferring elements would make one element copy outrank every picture the user copies for the rest of the session. So `set_annotation_clipboard` records `GetClipboardSequenceNumber()` alongside the payload, and `get_annotation_clipboard` reports `superseded` — the system clipboard has been written since — which is what the editor branches on. On non-Windows the counter doesn't exist, `superseded` is always false, and paste behaves exactly as it did before.
- Two editors opened on the **same** file both save to it (last write wins) — opening from the gallery is not deduplicated.
- **Closing an editor with unsaved changes asks first.** Dirtiness is tracked by *reference*, not by content: every store action that edits the document replaces the `annotations` array (and a crop replaces `capturedImage`), so comparing them against the snapshot taken at load and at each successful save costs nothing per render — and undo pushes the *same* array reference onto its history stack, so undoing back to the saved state goes clean again instead of staying dirty. Stringifying to compare would mean re-serializing pasted pictures, which travel inline as `data:` URLs, on every keystroke. The confirm hangs off the window's own close request (`onCloseRequested`) rather than off the X button, so Escape, Alt+F4 and the taskbar's Close are all covered by the one path; Pin and Delete close through a flag that skips it, having already asked (Pin) or having nothing left to save (Delete). Note what registering that listener costs: Tauri prevents **every** close on a window that has a JS `tauri://close-requested` listener and leaves it to the frontend to call `destroy()` — hence `core:window:allow-destroy` in `capabilities/default.json`, without which an editor could not be closed at all.
...
- **The pixels travel inline, as a `data:` URL in `src`.** An annotation has to survive trips a blob URL can't: the sidecar JSON written next to a saved capture, and the copy/paste payload that crosses into *another editor window*, which is a different webview where this one's blob URLs resolve to nothing. The cost is sidecar size, paid once per distinct picture.
- **Decoding is asynchronous but `drawAnnotation` is not.** Bitmaps are cached by `src` (`getEmbeddedImage`), so the frame right after a paste draws a translucent placeholder and a load listener repaints the real thing — that placeholder is also what keeps an undecodable picture selectable (and therefore deletable) instead of invisible. Anything that renders the document in one synchronous pass must await `decodeEmbeddedImages` first, or a picture still in flight is flattened as an empty box: that's why `exportBlob` and `handleSave` do.
...
src-tauri/src/
  ...
  monitors.rs      — the only place that calls xcap::Monitor::all(); cross-checks the
                     list against SM_CMONITORS, retries, reports `complete`
...
**OCR shells out to an agentic coding CLI — either one.** No OCR engine is bundled; `run_ocr` writes the image to `%TEMP%\clipse-ocr` and runs whichever CLI `AppSettings.ocr.engine` selects (`"auto"` — Codex first, then Claude Code — `"codex"`, or `"claude"`). The two are invoked differently: `codex exec --image <path> <prompt>` takes the image as a flag, while **Claude Code has no image flag**, so the path goes in the prompt text and the file is read with the `Read` tool — which means `--allowedTools Read` (a permission prompt `-p` could never collect would otherwise fail the run) and a working directory set to the image's own folder, so no `--add-dir` grant is needed. That folder is a dedicated subdirectory rather than the temp root, since pointing an agentic CLI at all of `%TEMP%` makes it scan a pile of unrelated files for project context.

Two Windows details are load-bearing. `std::process::Command` resolves a bare program name by appending `.exe` only — it **never consults `PATHEXT`** — and both CLIs are commonly npm-installed, which ships `claude.cmd`/`codex.cmd` and no `.exe` at all; so `resolve()` walks `PATH` × `PATHEXT` itself and hands `Command` the full `.cmd` path, which Rust ≥1.77 runs through `cmd.exe` with correct argument escaping. And that `cmd.exe` needs `CREATE_NO_WINDOW`, or a console window sits on screen for the whole run — which is tens of seconds, not milliseconds. For the same reason the child is spawned on `spawn_blocking`: blocking an async-runtime worker for that long stalls unrelated work, since the capture paths run on those threads.
...
**Monitor enumeration goes through `monitors::enumerate()`, never `xcap::Monitor::all()` directly.** xcap 0.0.15 *silently drops* a monitor it fails to describe: `EnumDisplayMonitors` hands it every `HMONITOR`, each then goes through `EnumDisplaySettingsW(ENUM_CURRENT_SETTINGS)`, and one that fails there is left out of the list entirely — with only a `log::error!` that goes nowhere, since no log backend is installed. That call fails *transiently* while a display is mid-mode-change (DPMS wake, GPU driver reset, RDP/VDI reconnect, a cable going in), so the list comes back reporting only the primary while a second monitor is very much attached, and every consumer believes it: overlays get built for one display (the other cannot be selected on at all), the virtual screen shrinks to it, and `capture_rect_composited` zero-fills — leaves fully transparent — whatever the missing monitor covered. "Only the main display gets captured." Upstream fixed this in 0.9.x by making that path infallible; we are on 0.0.15.

`GetSystemMetrics(SM_CMONITORS)` answers the same question without touching `EnumDisplaySettingsW`, so it is an independent ground truth for how many monitors *should* have come back. `monitors::enumerate()` compares the two, retries briefly (the race clears in milliseconds), and returns `complete: false` plus a `monitors: DEGRADED` line in `clipse.log` when the list is still short. Two rules follow from it:

- **Never persist a layout that isn't `complete`.** `open_overlay_inner` skips storing the pool signature and `prewarm_overlays` declines to build at all. Storing one is what turns a momentary drop-out into a permanent one: the stored signature matches the still-degraded list, so every later capture fast-paths onto a pool with no overlay for the missing monitor and nothing is left to notice. Prewarm is the dangerous one — it runs at startup and ~1s after `WM_DISPLAYCHANGE`, both squarely inside the flaky window.
- **One enumeration per capture, threaded through.** `virtual_screen_bounds_of` and `capture_region_png_of` take the monitor list instead of enumerating their own, and there is deliberately no enumerating wrapper for either. A rect sized from one enumeration and composited from another is exactly the mismatch that leaves a monitor's whole area transparent.
...
### Dragging captures out of the gallery

Dragging a gallery card drops the **file itself** into Explorer, a mail composer or a chat window (`commands/drag.rs`, Windows-only). A webview can't do this: an HTML5 drag payload is text, and WebView2 exposes no way to hand a drop target a file — so the *gesture* is detected in `Gallery.tsx` and the drag runs natively.

- **The gesture is mousedown + a movement threshold, not HTML5 `draggable`.** With `draggable` on, WebView2 would start its own drag on the same press and the two would race for the mouse capture; with it off it never starts one. `DRAG_THRESHOLD` (5px) separates a drag from a click. The pending drag is resolved *on mousedown* because the click handler that follows collapses the selection down to the clicked card — Explorer's rule, dragging a selected card drags the whole selection.
- **The data object is bound off an `IShellItemArray`, not built with `SHCreateDataObject`.** The latter is the obvious call and the wrong one: given a NULL parent folder and absolute PIDLs it returns a data object that **does not offer CF_HDROP**, so every plain shell target refuses the drop — the drag runs and the desktop shows a no-entry cursor, which looks like a broken drag but isn't. `SHCreateShellItemArrayFromIDLists` + `BindToHandler(BHID_DataObject)` yields the object Explorer itself hands out (CF_HDROP, FileContents, FileGroupDescriptorW, Shell IDList Array, ZoneIdentifier). `has_hdrop()` logs which of the two you actually got, since nothing else surfaces the difference — and its `QueryGetData` check compares against `S_OK` explicitly, because an *absent* format is reported as S_FALSE, a success HRESULT that `.is_ok()` would answer "yes" to.
- **`SHDoDragDrop` is modal and thread-bound**, so it is dispatched with `run_on_main_thread` and holds the main thread for the whole drag. That is what a drag is — Explorer blocks its own UI thread the same way — and it's invisible here because the gallery is idle meanwhile and WebView2 paints out of process. The IPC command waits on `spawn_blocking`, not on an async-runtime worker, since those are the threads the capture paths run on.
- **Only `DROPEFFECT_COPY` is offered.** A target allowed to ask for MOVE would delete the capture out of the gallery on a successful drop, with no undo.
- The picture under the cursor is built from the gallery's **cached thumbnail** (`storage::cached_thumb_path`), decoded off the main thread before the drag is dispatched — decoding a full-size capture there would delay the drag while the user is already moving the mouse, and a video has no full-size image to decode at all. Premultiplied alpha + `crColorKey = CLR_NONE` gives it Explorer's translucent look; failing to build one leaves a plain drop cursor, not a broken drag.
