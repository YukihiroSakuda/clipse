// The color palettes the annotation tools offer.
...
/** True for colors offered by the palettes above (as opposed to an eyedropper
 *  pick) — what decides whether a color is worth remembering as a default. */
export function isPaletteColor(hex: string): boolean {
  return Object.values(PALETTE).includes(hex) || TAILWIND_HEX_SET.has(hex)
}
